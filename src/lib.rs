...
///!
///! [@Github](https://github.com/shamansir/cayley-rust).
///!
///! To connect to a graph, start Cayley itself:
///!
///! `$ ./cayley http --dbpath=<your-database>`
///!
///! Then, bind to the graph this way:
///!
///! ```
...
///! ```
///!
///! For the moment, this code does no connection at all, when you only create a Graph.
...
///!
///! * [graph::Graph](./graph/struct.Graph.html) provides `.find(<Query>)` and `.save(<Morphism>)`;
///! * [path::Vertex](./path/struct.Vertex.html) provides:
///!     * [path::Path](./path/trait.Path.html) implemetation with `.Out(...)`, `.In(...)`, `.Has(...)`, `.Or(...)`, `.Follow(...)`, ...
///!     * [path::Query](./path/trait.Query.html) implemetation with `.All()`, `.GetLimit(...)`, ...
///! * [path::Morphism](./path/struct.Morphism.html) provides:
///!     * [path::Path](./path/trait.Path.html) implemetation with `.Out(...)`, `.In(...)`, `.Has(...)`, `.Or(...)`, `.Follow(...)`, ...
///!
